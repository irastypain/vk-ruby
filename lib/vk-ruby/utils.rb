...
module VK
  module Utils
    ...
    def self.camelize(name)
      ...
      words.each { |word| word.sub!(/^[a-z]/) { |s| s.upcase } }
    ...
    def self.stringify(object)
      case object
      when Hash
        object.each_with_object({}) { |(k, v), acc| acc[k.to_s] = stringify(v) }
      when Array
    ...
    def self.symbolize(object)
      case object
      when Hash
        object.each_with_object({}) { |(k, v), acc| acc[k.to_sym] = v.is_a?(String) ? v : symbolize(v) }
      when Array
    ...
    def self.stringify_arrays(params, separator = ',')
      changed_params = Hash[params.select { |_k, v| v.is_a?(Array) }.map { |k, v| [k, v.compact.join(separator)] }]
